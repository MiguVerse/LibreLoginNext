...
package xyz.miguvt.libreloginnext.common.premium;
...
public class AuthenticPremiumProvider implements PremiumProvider {
    ...
    private final AuthenticLibreLoginNext<?, ?> plugin;

    public AuthenticPremiumProvider(AuthenticLibreLoginNext<?, ?> plugin) {
        this.plugin = plugin;
    ...
    private PremiumUser getUserFromAshcon(String name) throws PremiumException {
        try {
            ...
            switch (connection.getResponseCode()) {
                case 200 -> {
                    var data = AuthenticLibreLoginNext.GSON.fromJson(new InputStreamReader(connection.getInputStream()), JsonObject.class);
