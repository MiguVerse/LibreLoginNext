...
package xyz.miguvt.libreloginnext.common.event;
...
import xyz.miguvt.libreloginnext.api.event.PlayerBasedEvent;
...
public class AuthenticPlayerBasedEvent<P, S> implements PlayerBasedEvent<P, S> {
    ...
    private final LibreLoginNextPlugin<P, S> plugin;
    private final PlatformHandle<P, S> platformHandle;

    public AuthenticPlayerBasedEvent(@Nullable User user, @Nullable P player, LibreLoginNextPlugin<P, S> plugin) {
        this.plugin = plugin;
    ...
    @Override
    public LibreLoginNextPlugin<P, S> getPlugin() {
        return plugin;
    }
