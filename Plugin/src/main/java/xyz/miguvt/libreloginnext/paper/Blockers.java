...
import static xyz.miguvt.libreloginnext.common.config.ConfigurationKeys.ALLOWED_COMMANDS_WHILE_UNAUTHORIZED;
...
public class Blockers implements Listener {
    ...
    public Blockers(PaperLibreLoginNext plugin) {
    ...
    public void onTeleport(PlayerTeleportEvent event) {
        if (inLimbo(event.getPlayer())) {
            ...
            if (serverHandler.getLimboServers().contains(event.getTo().getWorld()) && !event.getPlayer().hasPermission("libreloginnext.limbo.access")) {
                event.setCancelled(true);
            }
        }
    }
